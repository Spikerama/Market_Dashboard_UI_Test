...
<body>
  ...
  <script>
    ...
    function drawSparkline(containerId, data) {
      ...
      let path = '';
      data.forEach((v,i) => { const x = (i/(data.length-1)) * w; const y = h - ((v - min) / range) * h; path += i === 0 ? `M${x},${y}` : ` L${x},${y}`; });
      const svg = `
    ...
    function interpolateColor(from, to, t) {
      ...
      const r = (c => (c>>16)&255)(f(from)), g = (c => (c>>8)&255)(f(from)), b = (c => c&255)(f(from));
      const r2 = (c => (c>>16)&255)(f(to)), g2 = (c => (c>>8)&255)(f(to)), b2 = (c => c&255)(f(to));
      const rn = Math.round(r + (r2 - r)*t), gn = Math.round(g + (g2 - g)*t), bn = Math.round(b + (b2 - b)*t);
      return `rgb(${rn},${gn},${bn})`;
    }
    ...
    async function refreshSPY() {
      ...
      try {
        ...
        if (cached?.data) {
          ...
          setStatus(document.getElementById('sp500-status'),'ERROR');
        }
      }
    }
    ...
    async function refreshYieldCurve() {
      ...
      try {
        ...
        if (cached?.data) {
          const spread = cached.data.spread; const inverted = cached.data.inverted;
          if (document.getElementById('yield')) document.getElementById('yield').textContent = `${spread.toFixed(2)}%`;
          const changeEl = document.getElementById('yield-change');
          if (changeEl) {
            const arrow = spread >= 0 ? '▲' : '▼';
          ...
          setCard('yield', null, null, false);
          document.getElementById('yield-updated').textContent='Updated: --';
          setStatus(document.getElementById('yield-status'),'ERROR');
        }
      }
    }
    ...
    async function refreshEstimatedBuffett() {
      ...
      try {
        ...
        const json = await res.json();
        if (json.error) throw new Error(json.error);
        const ratio = json.ratio; const overvalued = ratio >= 3.0;
        ...
        if (changeEl) { changeEl.textContent = overvalued ? '▲ Overvalued' : (ratio >= 2.5 ? '▲ Caution' : '▼ Normal'); changeEl.className = 'change ' + (overvalued ? 'up' : (ratio >= 2.5 ? 'up' : 'down')); }
        const inner = document.getElementById('estimated-buffett-threshold'); const label = document.getElementById('estimated-buffett-label');
        ...
        setStatus(document.getElementById('estimated-buffett-status'), overvalued ? 'ALERT' : 'OK');
        cacheSet('estimated-buffett', { ratio, overvalued });
        pushHistory('estimated-buffett', ratio);
        drawSparkline('estimated-buffett-spark', getHistory('estimated-buffett'));
      } catch (e) {
        console.error('Estimated Buffett error', e);
        const cached = cacheGet('estimated-buffett');
        if (cached?.data) {
          const ratio = cached.data.ratio; const overvalued = ratio >= 3.0;
          if (document.getElementById('estimated-buffett')) document.getElementById('estimated-buffett').textContent = `${ratio.toFixed(2)}%`;
          const changeEl = document.getElementById('estimated-buffett-change');
          if (changeEl) { changeEl.textContent = overvalued ? '▲ Overvalued' : (ratio >= 2.5 ? '▲ Caution' : '▼ Normal'); changeEl.className = 'change ' + (overvalued ? 'up' : (ratio >= 2.5 ? 'up' : 'down')); }
          const inner = document.getElementById('estimated-buffett-threshold'); const label = document.getElementById('estimated-buffett-label');
          ...
          setStatus(document.getElementById('estimated-buffett-status'),'ERROR');
        }
      }
    }
    ...
    async function refreshTSLA() {
      ...
      try {
        const q = await getTwelveQuote('TSLA'); const price = parseFloat(q.close); const pct = parseFloat(q.percent_change);
        setCard('tsla', price, pct, false); cacheSet('tsla',{ price,pct });
        ...
        if(cached?.data){
          ...
          setStatus(document.getElementById('tsla-status'),'ERROR');
        }
      }
    }
    ...
    async function refreshLithium() {
      ...
      try{
        const q=await getTwelveQuote('LIT'); const price=parseFloat(q.close); const pct=parseFloat(q.percent_change);
        setCard('lithium', price, pct,false); cacheSet('lithium',{ price,pct });
        ...
        if(cached?.data){
          ...
          setStatus(document.getElementById('lithium-status'),'ERROR');
        }
      }
    }
    ...
    async function refreshVIX() {
      ...
      try{
        const q=await getTwelveQuote('VIXY'); const price=parseFloat(q.close); const pct=parseFloat(q.percent_change);
        setCard('vix', price, pct,false); cacheSet('vix',{ price,pct });
    ...
    async function refreshVIXIndex() {
      ...
      try{
        ...
        if(cached?.data){
          ...
          setStatus(document.getElementById('vix-index-status'),'NO DATA');
        }
      }
    }
    ...
    function refreshRiskTemperature() {
      ...
      const needle = document.getElementById('risk-temp-needle');
      if (needle) {
